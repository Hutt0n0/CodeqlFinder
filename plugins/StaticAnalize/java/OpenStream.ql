...
where
  ...
  exists(MethodAccess m |
    sink.getNode().asExpr() = m.getQualifier() and (m.getMethod() instanceof UrlOpenConnectionMethod or m.getMethod() instanceof UrlOpenStreamMethod)
  )
  or
  sinkNode(sink.getNode(), "url-open-stream")
