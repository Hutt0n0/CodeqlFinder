...
where 
sink.getNode().asExpr() = any(XQueryPreparedExecuteCall xpec).getPreparedExpression() or
sink.getNode().asExpr() = any(XQueryExecuteCall xec).getExecuteQueryArgument() or
sink.getNode().asExpr() = any(XQueryExecuteCommandCall xecc).getExecuteCommandArgument()
select  
