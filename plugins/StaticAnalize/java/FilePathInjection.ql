...
where sink.getNode().asExpr() = any(PathCreation p).getAnInput() and not sink.getNode() instanceof NormalizedPathNode
select sink.toString(),sink.getNode().getEnclosingCallable(), sink.getNode().getEnclosingCallable().getFile().getAbsolutePath(),  "Possiable File Injection"
